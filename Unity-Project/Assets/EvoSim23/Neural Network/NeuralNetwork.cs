...
public class NeuralNetwork : ICloneable
{
    ...
    public static readonly int numInputs = 16;  // self (4) + sensors (4) + memory (4)
    ...
    readonly static Dictionary<MutationType, int> mutations = new()
    {
    ...
    static (int layer, int item) GetRandomElementIndex(List<Layer> layers, MutationType which)
    {
        ...
        for (int i = 0; i < layers.Count; i++)
        {
            total += which switch
            {
                ...
                MutationType.MEMORY => layers[i].Results.Length,
